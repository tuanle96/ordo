...
describe('SchemaCacheService', () => {
    ...
    it('builds conservative keys using tenant, user, lang, version, and model dimensions', () => {
        ...
        expect(key).toBe('ordo:schema:http%3A%2F%2F127.0.0.1%3A38421:odoo17:17:2:en_US:res.partner');
    });
    ...
    it('uses a fixed 1 hour ttl when caching schemas', async () => {
        ...
        await service.set(odooFixtures.tokenPayload, 'res.partner', schema);

        expect(redisService.setJson).toHaveBeenCalledWith(
            'ordo:schema:http%3A%2F%2F127.0.0.1%3A38421:odoo17:17:2:en_US:res.partner',
            schema,
            3600,
        );
    });
    ...
    it('fails open when Redis reads or writes fail', async () => {
        ...
        await expect(service.get(odooFixtures.tokenPayload, 'res.partner')).resolves.toBeNull();
        await expect(
            service.set(odooFixtures.tokenPayload, 'res.partner', schema),
