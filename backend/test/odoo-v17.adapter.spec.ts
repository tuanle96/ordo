...
describe('OdooV17Adapter', () => {
    ...
    it('fails closed on unsupported warning payloads', async () => {
        const odooRpcService = {
            runModelOnchangeWithSession: jest.fn().mockResolvedValue({ value: { name: 'Acme' }, warning: 'bad-warning-shape' }),
        };
        const adapter = new OdooV17Adapter(odooRpcService as never, {} as never, {} as never, {} as never);
    ...
    it('fails closed on unsupported value payloads', async () => {
        const odooRpcService = {
            runModelOnchangeWithSession: jest.fn().mockResolvedValue({ value: ['bad-value-shape'] }),
        };
        const adapter = new OdooV17Adapter(odooRpcService as never, {} as never, {} as never, {} as never);
    ...
    it('fails closed on unsupported domain payloads', async () => {
        const odooRpcService = {
            runModelOnchangeWithSession: jest.fn().mockResolvedValue({ value: { name: 'Acme' }, domain: 'bad-domain-shape' }),
        };
        const adapter = new OdooV17Adapter(odooRpcService as never, {} as never, {} as never, {} as never);
    ...
    it('loads list schema from tree and search views', async () => {
        ...
        const adapter = new OdooV17Adapter(odooRpcService as never, {} as never, {} as never, listSchemaBuilder as never);
    ...
    it('loads kanban schema from kanban and search views when available', async () => {
        const odooRpcService = {
            callKwWithSession: jest
                .fn()
                .mockResolvedValueOnce({
                ...
                .mockResolvedValueOnce({ arch: '<kanban string="Leads" default_group_by="stage_id"><templates><t t-name="kanban-box"><div><field name="name" /></div></t></templates></kanban>' })
                .mockResolvedValueOnce({ arch: '<search><field name="name" /></search>' }),
        };
        const kanbanSchemaBuilder = {
            build: jest.fn().mockReturnValue({
                model: 'crm.lead',
                title: 'Leads',
        ...
        const adapter = new OdooV17Adapter(odooRpcService as never, {} as never, kanbanSchemaBuilder as never, {} as never);

        await expect(adapter.getKanbanSchema(
            { cookieHeader: 'session_id=abc123', odooUrl: 'http://example.com' } as never,
            'crm.lead',
        )).resolves.toEqual({
            model: 'crm.lead',
    ...
    it('returns null when the kanban view is unavailable', async () => {
        const odooRpcService = {
            callKwWithSession: jest
                .fn()
                .mockResolvedValueOnce({ name: { type: 'char', string: 'Name' } })
                .mockRejectedValueOnce(new Error('no kanban view')),
        };
        const adapter = new OdooV17Adapter(odooRpcService as never, {} as never, {} as never, {} as never);
